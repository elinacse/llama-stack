...
class TestConstructModelFromIdentifier:
    def test_embedding_model_classified_by_name(self):
        adapter = _make_adapter()
        model = adapter.construct_model_from_identifier("nomic-embed-text-v1.5")

        assert model.model_type == ModelType.embedding

    def test_rerank_model_classified_by_name(self):
        adapter = _make_adapter()
        model = adapter.construct_model_from_identifier("bge-reranker-v2-m3")

        assert model.model_type == ModelType.rerank

    def test_other_model_falls_through_to_default(self):
        adapter = _make_adapter()
